...
STAGES: dict[PlanStage, dict[str, str]] = {
    ...
    PlanStage.principles: {
        ...
        "prompt": (
            ...
            "- '구현계획 및 현황관리'에 **커밋 규약**을 반드시 적어라: 커밋 메시지에 "
            "`task #<작업번호>`를 넣는다(예: `feat: 로그인 API 추가 (task #3)`). 플랫폼은 기본 "
            ...
            "하는지(requirements.txt·pyproject.toml·package.json·index.html), 프로필별로 "
            "어떻게 실행되는지. 플랫폼은 이 폴더 구조를 시그니처 파일로 감지해 배포 단위를 "
            "정한다(app/services/structure.py) — 문서와 리포가 어긋나면 배포가 문서와 다르게 "
            "된다. 컴포넌트가 하나뿐인 단순한 프로젝트라면 이 절은 짧게 한 줄로 족하다."
        ),
        "request": (
