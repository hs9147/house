...
class Settings(BaseSettings):
    ...
    # LLM 한 번 응답의 최대 출력 토큰. **보내지 않으면 프로바이더 기본값이 적용된다** —
    # 실측에서 그 기본값이 1024 수준이었고, 기획 산출물이 문장 중간에서 잘려 저장됐다
    # (supplier-pool: 기획서 1759자 · 아키텍처 1656자에서 끊김). C4 블록은 문서 끝에
    # 두라고 지시하므로 잘릴 때 가장 먼저 사라진다 — 시각화가 "미작성"으로 보인 원인이다.
    # 0이면 보내지 않는다(이 파라미터를 거부하는 엔드포인트용 탈출구).
    #
    # 8192로도 기획 산출물이 잘렸다(Opus, 실사용 보고) — 기획 문서는 표와 근거를 함께
    # 담아 길다. 최신 Claude·GPT 계열은 32k 이상 출력을 허용하므로 기본을 올린다. 모델이
    # 허용하지 않는 값을 보내면 프로바이더가 400으로 거부하니, 그 모델에서는 이 값을
