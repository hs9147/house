...
POST /paas/api/v1/mcp/graph                         # 사내 MCP 서버 — 문서 온톨로지(그래프·스키마, 읽기 전용)
                                                ...
                                                #   절은 제목 스타일 없는 평문 `제N조(제목)` 줄에서도 뽑힘
                                                #   (hwpx·pdf·97-2003 경유 추출에는 제목 단계가 없음) —
                                                #   병합 표(인라인 HTML)의 머리글도 스키마로 읽음
...
- **서버 디스크의 사내 문서 폴더 붙이기**: `PAAS_DOC_ROOTS`에 절대 경로를 적으면 그
  ...
  **항상 읽기 전용**입니다 — 플랫폼이 만든 것이 아니므로 쓰기·삭제 도구를 아예 광고하지
  ...
  없으면 깨진 글자 대신 이유를 돌려줍니다(무엇을 설치하면 되는지 함께).

  **PDF OCR 폴백(선택)**: 텍스트 레이어가 없는 스캔 PDF와, 추출은 되는데 한글이
  사설영역(PUA) 코드로 깨져 나오는 HWP 계열 PDF는 — 예전에는 뒤쪽이 쓰레기 텍스트로
  조용히 색인됐습니다 — 페이지를 이미지로 렌더링해(`pypdfium2`+`Pillow`, pip)
  Tesseract(`PAAS_TESSERACT_PATH`, kor 언어 데이터 포함)로 읽습니다. 텍스트가 멀쩡한
  PDF는 이 경로를 타지 않고, 도구가 없으면 해당 PDF만 "추출 불가"로 남습니다.
  > 측정(합성 스캔, 한글 규정): 150dpi + 단일 블록 모드(psm 6)에서 쪽당 ~1초, 문자
  > 일치 99%. **기본 페이지 분할은 같은 이미지에서 21~38%였습니다** — "물품 반출"이
  > "둘줌 반줄"이 되는 수준이라, psm 설정 하나가 이 기능의 성패를 가릅니다. 해상도를
  > 올리면 느려지기만 하고 정확도는 내려갔습니다(300dpi 쪽당 2.5초·95%). 실제
  > 스캔(기울어짐·도장)은 이보다 낮습니다 — 그래서 지금 0인 문서를 살리는 폴백으로만
  > 쓰고, 쪽수 상한(30쪽)을 두며, OCR로 추출된 문서는 `.ready` 마크다운 첫 줄에 출처
  > 주석이 남습니다(검색 평문에는 안 들어갑니다).
- **읽기용 마크다운과 `.ready` 캐시**(`services/docready.py`): 문서를 평문으로만 뽑으면
  **표가 셀 나열로 무너집니다** — "구분 / 산정 기준 / 적용 시점 / 국내 자재 / 직전 분기
  ...
  표는 마크다운 표(hwpx의 표 포함), 스프레드시트는 시트 이름 + 표가 됩니다.
  값이 제자리에 남게 하는 처리 셋 — 엑셀은 빈 셀을 XML에 아예 쓰지 않으므로 셀
  참조(`r="C2"`)로 제 열에 놓고(무시하면 금액이 담당자 열로 밀립니다), docx 세로
  병합의 이어지는 빈 셀은 위 행의 값을 내려 채우며(행마다 온전한 레코드가 되게),
  hwpx 표는 단락으로 흘리지 않고 행·열을 살립니다.
...
- **주기 갱신 + 모니터**(`services/scheduler.py`): 갱신이 필요한 일 — 외부 API 카탈로그
  ...
  **작업 목록은 사람이 만들지 않습니다.** 매 순회 첫 단계가 재조정입니다 — 저장소가 늘면
  ...
  한 번에 못 끝낸 작업은 **다음 주기까지 기다리지 않고 다음 순회에 이어서** 돕니다.
  색인은 호출마다 예산(20초)만큼만 진행하고 `done: false`로 답하는데, 그걸 15분 재우면
  듀티 사이클이 2%라 문서 수천 건짜리 저장소는 첫 색인이 하루를 넘깁니다.
- **업로드·삭제는 그 파일만 즉시 색인합니다**(`docsearch.index_one`/`forget_one`).
  주기 색인을 기다리면 방금 올린 문서가 최대 15분간 검색에 안 잡히고, 지운 문서는
  반대로 **검색에 계속 잡히다가 읽으러 가면 "파일이 없습니다"**가 납니다 — 검색이
  있다고 한 것을 읽지 못하는 상태입니다.

  갱신은 `storage.write_file`/`delete_file` **안에서** 일어납니다. 이 둘만 `root`가
  아니라 `Store`를 받는 이유가 그것입니다 — 창구가 넷(REST 업로드·삭제, MCP
  `write_file`·`delete_file`)인데 API 층에 훅을 흩뿌리면 다섯 번째가 생길 때 잊습니다
  (실제로 넷이 다 잊고 있었습니다).

  **전체 스캔의 대체가 아니라 지름길입니다.** 문서 폴더는 공유 폴더라 탐색기·SMB로
  직접 들어오는 파일은 이 플랫폼을 거치지 않습니다. 그래서 즉시 색인이 실패해도
  **업로드는 201입니다** — 파일은 이미 디스크에 있고, 여기서 500을 주면 사용자는 같은
  파일을 다시 올립니다. 늦어질 뿐 틀리지 않습니다. 큰 파일(10MB 초과)은 업로드 응답에
  추출 시간을 얹지 않으려고 주기 색인에 미룹니다.
