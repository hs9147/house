...
def test_bound_module_becomes_callable_agent_in_chat(monkeypatch):
    """바인딩한 모듈이 채팅에서 A2A 도구로 노출되고, 모델이 부르면 게이트웨이를 거쳐
    대상까지 실제로 왕복한다 — '사내 자원을 에이전트로 추상화한다'의 실증."""
    from app.services import llm as llm_service

    c = _client()
    pid = c.post("/paas/api/v1/projects", json={
        "name": "agent-app", "type": "python", "git_url": "https://git.example.com/o/x",
    }, headers=ADMIN).json()["id"]
    ...
    mid = _module(c, name="orders-db", type="database")
    assert c.post(f"/paas/api/v1/projects/{pid}/modules/{mid}/bind",
                  json={"env_prefix": "ORDERS"}, headers=ADMIN).status_code == 201
        ...
        if len(payloads) == 1:
            return {"choices": [{"message": {
                ...
                "tool_calls": [{"id": "c1", "function": {
                    "name": "a2a__orders-db",
                    "arguments": '{"capability": "execute_query", "input": {"sql": "select 1"}}',
                }}],
            }}]}
        return {"choices": [{"message": {"content": "주문 3건입니다."}}]}
    ...
    r = c.post(f"/paas/api/v1/chat/sessions/{sid}/messages",
               json={"content": "주문 몇 건이야?"}, headers=ADMIN)
    assert r.status_code == 200
    assert r.json()["reply"] == "주문 3건입니다."

    # 1. 카드가 도구로 노출됐고, 고를 수 있는 능력은 카드의 skills뿐이다
    tool = payloads[0]["tools"][0]["function"]
    assert tool["name"] == "a2a__orders-db"
    assert tool["parameters"]["properties"]["capability"]["enum"] == ["execute_query", "inspect_schema"]

    # 2. 모델의 호출이 게이트웨이를 거쳐 대상까지 갔고, 자격증명은 게이트웨이가 붙였다
    assert relayed["json"]["capability"] == "execute_query"
    assert relayed["json"]["params"] == {"sql": "select 1"}
    assert relayed["headers"]["authorization"] == "Bearer mk-1"

    # 3. 결과가 tool 메시지로 모델에 되돌아갔다
    assert any(m.get("role") == "tool" and "rows" in m.get("content", "")
