...
>
> **doctor가 통과하는데 브라우저만 실패한다면 경로가 다른 것이다.** doctor는 서버에서
> 돌고 브라우저는 사용자 PC에서 돈다 — 그 사이(사내 프록시 등)는 doctor가 지나가지
> 않는 구간이다. 그래서 doctor는 양쪽이 통과했을 때 **IIS 접근 로그**에서 터미널 소켓
> 경로를 찾아 보여 준다:
>
> - **로그에 101이 있다** — IIS는 업그레이드를 넘겼다. 그 뒤(백엔드·셸)를 본다.
> - **101이 아닌 상태코드가 있다** — IIS가 거기서 답했다. 그 코드가 원인이다.
> - **그 경로 요청이 아예 없다** — **요청이 IIS까지 오지도 않았다.** 브라우저와 서버
>   사이를 본다: Upgrade를 넘기지 않는 사내 HTTP 프록시, 또는 다른 호스트·바인딩.
>
> 서버 쪽 로그도 같이 본다. `/system/powershell/ws` 세션이 시작됐다면 서비스 stdout에
> `[paas] terminal session ended after ...` 한 줄이 반드시 남는다 — **그 줄이 없다는
> 것은 세션이 시작조차 못 했다는 뜻**이고, 그것만으로 백엔드는 용의선상에서 빠진다.
>
> **`-Proxy`는 브라우저가 실제로 여는 주소여야 한다.** 기본값은 `http://localhost`인데,
> 사이트가 호스트명으로 바인딩돼 있으면 `localhost`는 **다른 사이트**(보통 Default Web
> Site)로 간다 — 그러면 /paas 규칙이 없어 WebSocket이 실패하고, 원인과 무관하게
> "앞단이 업그레이드를 안 넘긴다"로 보인다. doctor는 소켓을 시도하기 전에 앞단으로
> **HTTP가 같은 백엔드에 닿는지** 먼저 확인해서 이 착각을 막는다(리비전을 대조한다).
>
